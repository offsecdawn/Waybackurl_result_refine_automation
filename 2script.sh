...
do
	response_code=`curl -k -i --silent  $i --parallel-max 50 --connect-timeout 3 | tac| tac | head -n 1 | cut -d' ' -f2 `
		...
		echo "========================================================================================================================================" >> grep_output.txt
 		...
 		curl -k -s $i |tac |tac | js-beautify | grep --color=always -i -E -- 'token|swagger|admin|jenkin|sandbox|wordpress|drupal|csrf|vpn' >> grep_output.txt
		echo "========================================================================================================================================" >> grep_output.txt
        fi
done
...
do
        ...
        then
                echo "========================================================================================================================================" >> grep_output.txt
                echo $i >> grep_output.txt
                curl -k -s $i |tac |tac | grep --color=always -i -E -- 'token|swagger|admin|jenkin|sandbox|wordpress|drupal|csrf|vpn' >> grep_output.txt
                echo "========================================================================================================================================" >> grep_output.txt
        fi
done
